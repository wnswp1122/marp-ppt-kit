...
- **테마 = 팔레트 교체** — 구조는 그대로 두고 색·폰트만 바꾸면 테마가 바뀐다. 기본 4종 제공.
...
`~/.bashrc`(또는 `~/.zshrc`)에 추가하면 명령이 짧아진다. 덱 이름 Tab 자동완성도 가능:

```bash
alias ppt='node ~/경로/marp-ppt-kit/build.mjs'    # ppt <덱> [fmt]
alias pv='node ~/경로/marp-ppt-kit/build.mjs'      # 라이브 뷰어용으로 --serve 조합
```
> 저장소엔 개인 별칭을 커밋하지 않는다. 본인 환경에서만 설정.
...
| `/review <deck>` | 검토만 | 별도 패스로 서사·과밀·흐름·디자인 진단 |
...
├── themes/<name>.css        # 팔레트 = 색·폰트(:root)만. tech·light·mono·aurora
