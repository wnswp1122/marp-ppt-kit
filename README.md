...
슬라이드 30장을 PowerPoint로 만들다 보면 — 도형 정렬 맞추고, 폰트 통일하고, 한 장 고치면 뒤가 다 틀어진다. **여기선 글만 쓰면 디자인은 시스템이 책임진다.**

- ✍️ **마크다운으로 쓴다** — 도형 미는 대신 내용에 집중. 텍스트라 `git diff`·버전관리·AI 생성이 자연스럽다.
- 🎨 **디자인은 테마가 맡는다** — 레이아웃·색·폰트를 CSS 시스템이 일관되게. `theme:` 한 줄로 다크↔라이트↔에디토리얼 교체(기본 5종).
- 🧩 **내용 ↔ 디자인 분리** — '말'과 '디자인'을 한 파일 안에서 구역으로 나눠, **글만 고쳐도 레이아웃이 안 깨진다.**
- 🔀 **장당 1파일** — 슬라이드 = `slides/NN-name.md` 하나. 병렬 작성·순서 변경·부분 수정이 쉽다.
- 📦 **결과는 파일 하나** — 이미지·JS까지 인라인된 **self-contained HTML**. 그냥 열거나 공유하면 끝(서버 불필요).
- ⚡ **HTML이라 살아있다** — 카운트업·막대·드래그 슬라이더·Mermaid 등 PPT로는 안 되는 **동적 슬라이드**.

> 대신, 픽셀 단위 자유 배치는 포기한다. "내용 + 시스템 디자인"에 집중하는 도구다.
