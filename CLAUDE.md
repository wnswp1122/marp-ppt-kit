...
node build.mjs <덱> editorial          # 덱 뒤에 테마명 바로 (= --theme=editorial) → dist/<덱>.editorial.html
node build.mjs <덱> --theme=editorial  # 같은 동작 (PPT_THEME=editorial 환경변수도 동일)
```
...
- 단축 별칭(개인 `~/.bashrc`): `ppt <덱> [테마|fmt]`(빌드) · `pv <덱> [테마]`(라이브 뷰어, 기본 포트 1122) · `po <덱> [테마]`(빌드된 `dist/<덱>[.테마].html` 열기) · `pptime <덱>`(예상 발표 시간) · `use <덱>`(활성 덱 지정) · `deck`(현재 활성 덱). **덱·테마 모두 Tab 자동완성**(2번째 인자에 테마/포맷).
