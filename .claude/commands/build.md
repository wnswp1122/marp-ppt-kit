---
description: 슬라이드 파일들을 합쳐 HTML/PDF/PPTX로 빌드한다
argument-hint: <deck-name> [html|pdf|pptx]
---
...
2. 실행:
   ...
   - PDF: `node build.mjs <deck-name> pdf`
   - PPTX: `node build.mjs <deck-name> pptx`
   - 전체: `node build.mjs all`
3. 에러 없이 `dist/<deck-name>.<ext>` 생성됐는지 확인하고 경로를 보고한다.
